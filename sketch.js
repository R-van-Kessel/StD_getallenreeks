...
// ============================================
// ALLE REEKSTYPEN - 25 stuks op vergelijkbaar niveau
// ============================================
const sequenceTypes = [
    // --- ORIGINELE REEKSEN ---
    { name: "Driehoeksgetallen",  generate: (n) => (n * (n + 1)) / 2,             length: 5 },
    { name: "Kwadraten",          generate: (n) => (n + 1) * (n + 1),             length: 4 },
    { name: "Machten van 2",      generate: (n) => Math.pow(2, n),                length: 5 },
    { name: "Fibonacci",          generate: (n) => {
        if (n <= 1) return n;
        let a = 0, b = 1;
        for (let i = 2; i <= n; i++) { let t = a + b; a = b; b = t; }
        return b;
    }, length: 6 },
    { name: "Oneven getallen",    generate: (n) => 2 * n + 1,                     length: 5 },
    { name: "Kubussen",           generate: (n) => (n + 1) * (n + 1) * (n + 1),  length: 4 },
    { name: "Stap +3",            generate: (n) => 2 + n * 3,                     length: 5 },
    { name: "Stap +5",            generate: (n) => 3 + n * 5,                     length: 5 },
    { name: "Verdubbelen -1",     generate: (n) => Math.pow(2, n) - 1,            length: 5 },
    { name: "Driehoek x2",        generate: (n) => n * (n + 1),                   length: 5 },
    // --- NIEUWE REEKSEN ---
    { name: "Machten van 3",      generate: (n) => Math.pow(3, n),                length: 4 },
    { name: "Even getallen",      generate: (n) => 2 * (n + 1),                   length: 5 },
    { name: "Stap +7",            generate: (n) => 1 + n * 7,                     length: 5 },
    { name: "Stap +4",            generate: (n) => 5 + n * 4,                     length: 5 },
    { name: "Stap +6",            generate: (n) => 4 + n * 6,                     length: 5 },
    { name: "Kwadraten +1",       generate: (n) => (n + 1) * (n + 1) + 1,        length: 4 },
    { name: "Verdubbelen +3",     generate: (n) => Math.pow(2, n) + 3,            length: 5 },
    { name: "Vijfhoeksgetallen",  generate: (n) => (n * (3 * n - 1)) / 2,        length: 5 },
    { name: "Zeshoeksgetallen",   generate: (n) => n * (2 * n - 1),               length: 5 },
    { name: "Stap x2 +1",         generate: (n) => Math.pow(2, n + 1) - 1,       length: 5 },
    { name: "Driehoek +n",        generate: (n) => (n * (n + 1)) / 2 + n,        length: 5 },
    { name: "Kwadraten -n",       generate: (n) => (n + 1) * (n + 1) - n,        length: 4 },
    { name: "Machten van 2 x3",   generate: (n) => Math.pow(2, n) * 3,            length: 5 },
    { name: "Stap +9",            generate: (n) => 6 + n * 9,                     length: 5 },
    { name: "Stap +11",           generate: (n) => 3 + n * 11,                    length: 5 }
];
...
function generateQuestions() {
    ...
    // Shuffle de reekstypen voor variatie
    let shuffledTypes = [...sequenceTypes];
    for (let i = shuffledTypes.length - 1; i > 0; i--) {
        let j = Math.floor(Math.random() * (i + 1));
        [shuffledTypes[i], shuffledTypes[j]] = [shuffledTypes[j], shuffledTypes[i]];
    }
    
    // Bijhouden welke vraagteksten en antwoorden al gebruikt zijn
    ...
    while (questions.length < 10 && attempts < maxAttempts) {
        ...
        // Roteer door alle reekstypen zodat we veel variatie hebben
        let sequenceType = shuffledTypes[typeIndex % shuffledTypes.length];
        typeIndex++;
        
        let startOffset = Math.floor(Math.random() * 4);
        let questionPosition = Math.floor(Math.random() * 3);
        ...
        if (questionPosition === 0) {
            // Vraagteken aan het BEGIN: ?-3-6-10-15
            answer = sequenceType.generate(startOffset);
            for (let j = startOffset + 1; j <= startOffset + sequenceType.length; j++) {
                sequence.push(sequenceType.generate(j));
            }
            questionText = "?-" + sequence.join("-");
            
        } else if (questionPosition === 1) {
            // Vraagteken in het MIDDEN
            let midPoint = Math.floor(sequenceType.length / 2);
            for (let j = startOffset; j < startOffset + midPoint; j++) {
                sequence.push(sequenceType.generate(j));
            }
            answer = sequenceType.generate(startOffset + midPoint);
            for (let j = startOffset + midPoint + 1; j <= startOffset + sequenceType.length; j++) {
                sequence.push(sequenceType.generate(j));
            }
            questionText = sequence.slice(0, midPoint).join("-") + "-?-" + sequence.slice(midPoint).join("-");
            
        } else {
            // Vraagteken aan het EIND
            for (let j = startOffset; j < startOffset + sequenceType.length; j++) {
                sequence.push(sequenceType.generate(j));
            }
            answer = sequenceType.generate(startOffset + sequenceType.length);
            questionText = sequence.join("-") + "-?";
        }
        
        // ============================================
        // DUBBELEN VOORKOMEN: sla over als vraag of antwoord al bestaat
        // ============================================
        if (usedQuestionTexts.has(questionText)) continue;
        if (usedAnswers.has(answer)) continue;
        
        // Unieke vraag gevonden, voeg toe
        usedQuestionTexts.add(questionText);
        usedAnswers.add(answer);
        questions.push({ text: questionText, answer: answer });
        answers.push(answer);
    }
    
    // Shuffle antwoorden voor de oranje blokken
    answers = shuffle(answers);
    
    // Maak vraagblokken (rijen 1-2)
    let questionIndex = 0;
    ...
    // Maak antwoordblokken (rijen 3-4)
    let answerIndex = 0;
...
function draw() {
    ...
    for (let block of blocks) {
        ...
        if (!IS_MOBILE && !showDinoGame && block.isQuestion && block.row >= 1 && block.row < 3 &&
            !draggingBlock && mouseX >= block.x && mouseX <= block.x + CELL_SIZE &&
    ...
    if (draggingBlock) drawBlock(draggingBlock);
    ...
    if (canvasButtons.length > 1) canvasButtons[1].label = 'Score: ' + correctCount + '/10';
    ...
    if (!showDinoGame && !showHandCursor) {
        for (let block of blocks) {
            if (block.isQuestion && block.row >= 1 && block.row < 3 &&
                ...
                mouseY >= block.y && mouseY <= block.y + CELL_SIZE) {
                showHandCursor = true; break;
            }
        }
    }
    if (draggingBlock) showHandCursor = true;
    if (!IS_MOBILE) setCursor(showHandCursor ? 'pointer' : 'default');
...
function drawBlock(block) {
  ...
  // Verkort tekst als hij te breed is (HIER, vóór de if/else)
  let displayText = block.text;
  let maxWidth = CELL_SIZE - 20;
  while (textWidth(displayText) > maxWidth && displayText.includes('-')) {
  ...
  if (block.isQuestion && block.hoverProgress > 0) {
    text(displayText, CELL_SIZE / 2, CELL_SIZE / 2);
  } else {
    text(displayText, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2);
  }
...
function checkAnswers() {
    ...
    if (correctCount === 10) {
        isFlashing = true;
        flashCounter = 0;
        if (canvasButtons.length > 1) canvasButtons[1].color = color(255, 215, 0);
    } else {
        if (canvasButtons.length > 1) canvasButtons[1].color = color(156, 39, 176);
    }
}
